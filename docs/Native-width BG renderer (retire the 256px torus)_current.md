...
3. **Blit offset (the smooth-scroll math).** The window's top-left within the
...
```
Checks:
1. **Wrap gone:** re-run the cols-256–319-vs-0–63 comparison (the script in the
   2026-06-16 session); expect it to now diverge like a normal control
   (~40–50% different), proving 40 distinct columns.
2. **No top-row garbage** at `DEBUG_WALK_STEPS=1,2,3` (the reproducer).
3. **Baseline unchanged:** `DEBUG_TRANSITION=1 DEBUG_BASELINE=1` still renders
   clean Pallet Town.
4. **Smoothness:** capture mid-step (odd `DEBUG_WALK_STEPS`) shows a 2px-offset
   glide, not a tile snap. Calibrate the `Xoff/Yoff` sign here.
5. **Interactive:** `make SKIP_TITLE=1`, launch DOSBox-X, walk all four
